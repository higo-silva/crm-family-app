...
def render_overview_dashboard(current_username, df_all_transactions):
    ...
    if not df_all_transactions.empty:
        ...
        ].copy() # Adicionar .copy() para evitar SettingWithCopyWarning

        if not df_filtered.empty:
            df_filtered['AnoMes'] = df_filtered['Data'].dt.to_period('M').astype(str)
            
            monthly_summary = df_filtered.groupby(['AnoMes', 'Tipo'])['Valor'].sum().unstack(fill_value=0)
            monthly_summary['Saldo'] = monthly_summary.get('receita', 0) - monthly_summary.get('despesa', 0)
            
            # Reindexar para garantir todos os meses no período
            all_months = pd.period_range(start_date, end_date, freq='M')
            monthly_summary = monthly_summary.reindex(all_months.astype(str), fill_value=0)
            
            fig = px.line(monthly_summary, x=monthly_summary.index, y=['receita', 'despesa', 'Saldo'],
...
if st.session_state['logged_in']:
    ...
    # Navegação por abas
    tab_overview, tab_transactions, tab_add_entry, tab_analysis, tab_planning = st.tabs([
        "📊 Visão Geral", "📝 Transações", "➕ Adicionar Lançamento", "📈 Análises Detalhadas", "🎯 Planejamento"
    ])

    current_username = st.session_state['username']
    all_transactions_data = get_transactions(current_username) # Carrega todas as transações uma vez
    # Converte para DataFrame aqui para ser usado por todas as abas
